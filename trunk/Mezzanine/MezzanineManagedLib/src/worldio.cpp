...
void setnames(char *name)
{
    ...
    sprintf_s(bakname)("packages/%s/%s_%d.BAK",   pakname, mapname, GameInit::LastMillis);
...
inline bool nhf(sqr *s) { return s->type!=BlockTypes::FHF && s->type!=BlockTypes::CHF; };
...
{
    sqr *o[4];
    o[0] = SWS(s,0,-1,GameInit::SSize);
    o[1] = SWS(s,0,1,GameInit::SSize);
    o[2] = SWS(s,1,0,GameInit::SSize);
    o[3] = SWS(s,-1,0,GameInit::SSize);
    wf = true;
    uf = true;
    if(SOLID(s))
    {
...
void toptimize() // FIXME: only does 2x2, make atleast for 4x4 also
{
    bool wf[4], uf[4];
    sqr *s[4];
    for(int x = 2; x<GameInit::SSize-4; x += 2) for(int y = 2; y<GameInit::SSize-4; y += 2)
    {
        s[0] = S(x,y);
        int wt = s[0]->wtex, ut = s[0]->utex;
        topt(s[0], wf[0], uf[0], wt, ut);
        topt(s[1] = SWS(s[0],0,1,GameInit::SSize), wf[1], uf[1], wt, ut);
        topt(s[2] = SWS(s[0],1,1,GameInit::SSize), wf[2], uf[2], wt, ut);
        topt(s[3] = SWS(s[0],1,0,GameInit::SSize), wf[3], uf[3], wt, ut);
        loopi(4)
        {
            if(wf[i]) s[i]->wtex = wt;
            if(uf[i]) s[i]->utex = ut;
...
void save_world(char *mname)
{
    ...
    hdr.numents = 0;
    loopv(ents) if(ents[i].type!=StaticEntity::NOTUSED) hdr.numents++;
    header tmp = hdr;
    ...
    loopv(ents) 
    {
        if(ents[i].type!=StaticEntity::NOTUSED) 
        {
            entity tmp = ents[i];
...
void load_world(char *mname)        // still supports all map formats that have existed since the earliest cube betas!
{
    ...
    loopi(hdr.numents)
    {
        ...
        e.spawned = false;
        if(e.type==StaticEntity::LIGHT)
        {
            if(!e.attr2) e.attr2 = 255;  // needed for GameInit::MapVersion<=2
    ...
    loopk(GameInit::CubicSize)
    {
        ...
        switch(type)
        {
            ...
            case 254: // only in GameInit::MapVersion<=2
            {
            ...
            default:
            {
                if(type<0 || type>=BlockTypes::MAXTYPE)
                {
...
COMMANDN(savemap, save_world, Support::FunctionSignatures::ARG_1STR);
