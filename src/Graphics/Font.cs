...
namespace SdlDotNet.Graphics
{
    ...
    public class Font : BaseSdlResource
    {
        ...
        /// <param name="fileName">Font filename</param>
        /// <param name="pointSize">Size of font</param>
        /// <param name="index">Choose a font face from a multiple font face containing file. The first face is always index 0.</param>
        public Font(string fileName, int pointSize, long index) {
            if (!Font.IsFontSystemInitialized) {
                Font.InitializeFontSystem();
            }
        ...
        /// <param name="pointSize">Size of font</param>
        /// <param name="index">Choose a font face from a multiple font face containing file. The first face is always index 0.</param>
        public Font(byte[] array, int pointSize, long index) {
            if (array == null) {
                throw new ArgumentNullException("array");
            }
            if (!Font.IsFontSystemInitialized) {
        ...
        /// <summary>
        /// Render Glyph to Solid
        /// </summary>
        /// <param name="glyph">Character to display</param>
        /// <param name="color">Color of text</param>
        /// <returns>Surface containing the character</returns>
        private Surface RenderGlyphSolid(char glyph, Color color) {
            Sdl.SDL_Color colorSdl = SdlColor.ConvertColor(color);
            IntPtr handle = SdlTtf.TTF_RenderGlyph_Solid(this.Handle, (short)glyph, colorSdl);
            GC.KeepAlive(this);
        ...
        /// </summary>
        /// <returns></returns>
        private Surface RenderGlyphShaded(
            char glyph, Color textColor, Color backgroundColor) {
            Sdl.SDL_Color textColorSdl =
                SdlColor.ConvertColor(textColor);
            Sdl.SDL_Color backgroundColorSdl =
                SdlColor.ConvertColor(backgroundColor);
        ...
        public bool FixedWidth {
            get {
                int result = SdlTtf.TTF_FontFaceIsFixedWidth(this.Handle);
                GC.KeepAlive(this);
                return (result > 0);
            }
        }
        ...
        /// <returns></returns>
        public GlyphData GetGlyphMetrics(char glyph) {
            int xMin;
            int xMax;
            int yMin;
            int yMax;
            ...
            SdlTtf.TTF_GlyphMetrics(this.Handle, (short)glyph, out xMin, out xMax, out yMin, out yMax, out advance);
            GC.KeepAlive(this);
            return new GlyphData(glyph, xMin, xMax, yMin, yMax, advance);
        }
        ...
        /// </summary>
        /// <param name="glyph">The glyph.</param>
        /// <param name="glyphColor">Color of the glyph.</param>
        /// <param name="antiAlias">if set to <c>true</c> [anti alias].</param>
        /// <returns></returns>
        public Surface RenderGlyph(char glyph, Color glyphColor, bool antiAlias) {
            return this.RenderGlyph(glyph, glyphColor, Color.Empty, antiAlias);
        }
        ...
        /// <returns></returns>
        public Surface RenderGlyph(char glyph, Color glyphColor, Color backgroundColor) {
            return this.RenderGlyph(glyph, glyphColor, backgroundColor, true);
        }
        ...
        /// <param name="glyphColor">Color of the glyph.</param>
        /// <param name="backgroundColor">Color of the background.</param>
        /// <param name="antiAlias">if set to <c>true</c> use anti alias.</param>
        /// <returns></returns>
        public Surface RenderGlyph(char glyph, Color glyphColor, Color backgroundColor, bool antiAlias) {
            if (backgroundColor.IsEmpty) {
                if (antiAlias) {
                    return this.RenderGlyphBlended(glyph, glyphColor);
                } else {
                    return this.RenderGlyphSolid(glyph, glyphColor);
                }
            } else {
                return this.RenderGlyphShaded(glyph, glyphColor, backgroundColor);
            }
        }
