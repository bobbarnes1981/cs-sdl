...
#region Constructors and Destructors
	...
	{
	    this.bitmapArray = new byte[(this.Width * this.Height * this.BytesPerPixel) + this.BmpHeader];
	    int result = 
		Sdl.SDL_SaveBMP_RW(this.Handle, Sdl.SDL_RWFromMem(bitmapArray, bitmapArray.Length), 1);
	    if (result != (int) SdlFlag.Success)
	    {
		throw SdlException.Generate();
	    }
	    stream.Write(bitmapArray, 0, bitmapArray.Length);
	    surfaceBitmap = (Bitmap)Bitmap.FromStream(stream);
	    this.resetBitmap = false;			
	}
