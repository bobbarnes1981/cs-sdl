...
public class CardArea extends CardPanelContainer implements CardPanelMouseListener {
	...
	public void layout () {
		...
		if (isVertical) {
			while (true) {
				...
				cardSpacingX = (int)Math.round(cardWidth * VERT_CARD_SPACING_X);
				cardSpacingY = cardHeight + (int)Math.round(cardWidth * VERT_CARD_SPACING_Y);
				...
				int availableCardsPerRow = (int)Math.floor((availableRowHeight - (cardHeight - cardSpacingY)) / cardSpacingY);
			...
			for (CardPanel panel : cardPanels) {
				...
				maxHeight = Math.max(maxHeight, Math.round(y + (cardHeight - cardSpacingY) + GUTTER_Y));
			...
			while (true) {
				cardHeight = Math.round(cardWidth * CardPanel.ASPECT_RATIO);
				int extraCardSpacingX = (int)Math.round(cardWidth * HORIZ_CARD_SPACING_X);
				cardSpacingY = (int)Math.round(cardHeight * HORIZ_CARD_SPACING_Y);
				cardSpacingX = cardWidth + extraCardSpacingX;
				int maxRows = (int)Math.floor((cardAreaHeight - GUTTER_Y * 2 + cardSpacingY) / (cardHeight + cardSpacingY));
				if (this.maxRows > 0) maxRows = Math.min(this.maxRows, maxRows);
	...
	public void mouseDragged (CardPanel dragPanel, int dragOffsetX, int dragOffsetY, MouseEvent evt) {
		...
		int mouseX = evt.getX();
		int mouseY = evt.getY();
		lastMouseX = mouseX + dragOffsetX;
		lastMouseY = mouseY + dragOffsetY;
