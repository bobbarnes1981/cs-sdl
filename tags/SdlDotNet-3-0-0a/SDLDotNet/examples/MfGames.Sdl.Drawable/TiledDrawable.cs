...
namespace MfGames.Sdl.Drawable
{
  ...
  /// or more parts. This is intended to work with the CachedDrawable
  /// class. The individual parts are converted into multiple frames.
  /// </summary>
  public class TiledDrawable : LoggedObject, IDrawable
  {
	...
	Rectangle sRect = new Rectangle(new Point(sx, sy),
					new Size((int) width, (int) height));
	//s1.FillRect(dRect, Color.Transparent);
	//s1.FillRect(dRect, Color.Black);
	//s1.SetAlpha(Alpha.Source, 255);
	...
	{
	  s1.DrawPixel(i, 0, Color.Blue);
	  s1.DrawPixel(i, 63, Color.Blue);
	}

	for (int i = 0; i < height; i++)
	{
	  s1.DrawPixel(0, i, Color.Blue);
	  s1.DrawPixel(63, i, Color.Blue);
	}
	*/
    ...
    /// drawable.
    /// </summary>
    public int FrameCount { get { return rows * cols; } }
    ...
    public Dimension2 Size
    {
      ...
      {
	if (value == null)
	  throw new DrawableException("Cannot assign null size");
